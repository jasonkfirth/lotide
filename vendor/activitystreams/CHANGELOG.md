...
- Lotide local fork: update the vendored crate baseline to
  `0.8.0-alpha.25`.
- Add `activitystreams::conformance` for strict compact-JSON validation of
  ActivityStreams document roots, common property ranges, context presence, and
  W3C known-bad fixture cases.
- Vendor the W3C ActivityStreams test corpus and add tests that parse accepted
  fixtures, reject fail fixtures through strict validation, and document known
  corpus deviations.
- Keep the parser tolerant for deployed federation compatibility while making
  strict validation available to tests and defensive callers.
- Accept older ActivityStreams context aliases with a trailing fragment during
