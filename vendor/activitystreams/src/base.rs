...
impl From<url::Url> for AnyBase {
    fn from(id: url::Url) -> Self {
        Self::from_xsd_any_uri(id.as_str().parse().expect("url::Url must be a valid IRI"))
    }
}
...
impl From<url::Url> for OneOrMany<AnyBase> {
    fn from(xsd_any_uri: url::Url) -> Self {
        Self::from_xsd_any_uri(
            xsd_any_uri
                .as_str()
                .parse()
                .expect("url::Url must be a valid IRI"),
        )
    }
}
