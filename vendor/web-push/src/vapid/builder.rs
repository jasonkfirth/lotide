...
impl<'a> VapidSignatureBuilder<'a> {
    ...
    pub fn from_pem_no_sub<R: Read>(pk_pem: R) -> Result<PartialVapidSignatureBuilder, WebPushError> {
        ...
        Ok(PartialVapidSignatureBuilder { key: pr_key })
    }
    ...
    pub fn from_base64(
        ...
        let pr_key = VapidKey::from_bytes(
    ...
    fn from_ec(ec_key: VapidKey, subscription_info: &'a SubscriptionInfo) -> VapidSignatureBuilder<'a> {
        VapidSignatureBuilder {
            claims: Claims::with_custom_claims(BTreeMap::new()),
            key: ec_key,
            subscription_info,
        }
    }
    ...
    pub(crate) fn read_pem<R: Read>(mut input: R) -> Result<VapidKey, WebPushError> {
        ...
        if found_sec1 {
            ...
            Ok(VapidKey::from_bytes(&key.key).map_err(|_| WebPushError::InvalidCryptoKeys)?)
        } else if found_pkcs8 {
            Ok(VapidKey::from_pkcs8_pem(&buffer).map_err(|_| WebPushError::InvalidCryptoKeys)?)
        } else {
